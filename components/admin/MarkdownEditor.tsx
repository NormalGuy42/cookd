...
export default function MarkdownEditor({ value, onChange, placeholder }: MarkdownEditorProps) {
  ...
  return (
    <div className="border border-[var(--border-light)] rounded-2xl overflow-hidden bg-white">
      <div className="flex items-center gap-1 p-2 border-b border-[var(--border-light)] bg-[var(--surface-cream)] flex-wrap">
        {toolbarButtons.map((button, index) => {
          ...
          return (
            <button
              ...
              title={button.title}
              className="p-2 text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-white rounded-lg transition-colors"
            >
              <Icon size={16} />
            </button>
          );
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-sm font-bold transition-colors ${
            isPreview 
              ? 'bg-[var(--cookd-orange)] text-white' 
      ...
      {isPreview ? (
        ...
        <textarea
          ...
          className="w-full min-h-[400px] p-4 bg-transparent text-[var(--text-primary)] font-mono text-sm resize-y focus:outline-none placeholder:text-[var(--text-muted)]"
        />
      )}
    </div>
  );
