...
import { User, Trophy, Loader2 } from 'lucide-react';
...
export default function HallOfFame() {
        ...
        <section className="py-16 px-6">
          <div className="max-w-6xl mx-auto text-center">
            <div className="flex items-center justify-center gap-3 text-[var(--text-secondary)]">
              <Loader2 size={24} className="animate-spin text-[var(--cookd-orange)]" />
              Loading...
            </div>
          </div>
        </section>
  ...
  return (
    <div className="min-h-screen bg-[var(--surface-cream)] text-[var(--text-primary)]">
      <section className="py-16 px-6">
        <div className="max-w-6xl mx-auto text-center">
          <h1 className="text-5xl md:text-6xl font-bold mb-4 text-[var(--text-primary)] font-display flex items-center justify-center gap-4">
            <Trophy size={48} className="text-[var(--cookd-golden)]" />
            Hall of Fame
          </h1>
          <p className="text-[var(--text-secondary)] text-lg">
            {hofProjects.length === 0
              ? 'Reserve your spot among the best projects.'
              : 'The best projects on Cookd, ranked by position.'}
          </p>
        </div>
      </section>
        ...
        <section className="py-8 px-6 pb-24">
          <div className="max-w-6xl mx-auto space-y-10">
            ...
            <div className="flex justify-center">
                ...
                <div className="relative w-full min-w-[260px] md:w-[820px] h-[220px] rounded-2xl border-4 border-dashed border-[var(--cookd-golden)] bg-[var(--cookd-golden)]/5 grid place-items-center hover:border-[var(--cookd-orange)] hover:bg-[var(--cookd-orange)]/5 transition-all">
                  <div className="text-center">
                    <div className="text-5xl font-extrabold text-[var(--cookd-golden)] font-display">$300</div>
                    <div className="text-sm text-[var(--text-muted)] mt-2">Premium</div>
                  </div>
                </div>
              </div>
            </div>
            <div className="mt-2 text-center text-3xl font-bold text-[var(--cookd-golden)] font-display">1</div>
              ...
              {Array.from({ length: 9 }).map((_, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center">
                      <div className="w-full cursor-pointer rounded-xl border-2 border-dashed border-[var(--border-medium)] bg-white h-40 grid place-items-center hover:border-[var(--cookd-orange)] hover:bg-[var(--cookd-orange)]/5 transition-all" onClick={() => handleCardClick(num)}>
                        <div className="text-center">
                          <div className="text-3xl font-bold text-[var(--text-secondary)] font-display">$150</div>
                          <div className="text-xs text-[var(--text-muted)] mt-1">Standard</div>
                        </div>
                      </div>
                    <div className="mt-2 text-lg font-bold text-[var(--text-primary)]">{num}</div>
                  </div>
                );
              })}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-9 gap-4">
              {Array.from({ length: 90 }).map((_, i) => (
                <div key={i} className="flex flex-col items-center">
                    <div className="w-full cursor-pointer h-24 rounded-xl border border-dashed border-[var(--border-light)] bg-white grid place-items-center hover:border-[var(--cookd-orange)] hover:bg-[var(--cookd-orange)]/5 transition-all" onClick={() => handleCardClick(i + 12)}>
                      <div className="text-xl text-[var(--text-muted)]">$75</div>
                  </div>
                  <div className="mt-2 text-xs text-[var(--text-muted)]">{i + 11}</div>
                </div>
              ))}
            </div>
          </div>
              ...
              {positionMap.get(1) ? (
                <div className="w-full md:w-[820px] cursor-pointer">
                  ...
                  <div className="mb-2 text-center">
                    <span className="text-3xl mt-2 text-center font-bold text-[var(--cookd-golden)] font-display">1</span>
                  </div>
                </div>
              ) : (
                <div className="cursor-pointer" onClick={() => handleCardClick(1)}>
                    ...
                    {project ? (
                      <div className="w-full cursor-pointer">
                        ...
                        <div className="w-full mt-2 text-center">
                          <span className="text-lg font-bold text-[var(--text-primary)]">{position}</span>
                        </div>
                      </div>
                    ) : (
                      <div className="w-full cursor-pointer" onClick={() => handleCardClick(position)}>
                        <div className="w-full rounded-xl border-2 border-dashed border-[var(--border-medium)] bg-white h-[280px] grid place-items-center hover:border-[var(--cookd-orange)] hover:bg-[var(--cookd-orange)]/5 transition-all">
                          <div className="text-center">
                            <div className="text-3xl font-bold text-[var(--text-secondary)] font-display">$150</div>
                            <div className="text-xs text-[var(--text-muted)] mt-1">Standard</div>
                          </div>
                        </div>
                        <div className="mt-2 text-center text-lg font-bold text-[var(--text-primary)]">{position}</div>
                      </div>
                    )}
                  </div>
                );
                  ...
                  <div key={position} className="flex flex-col items-center">
                    {project ? (
                      <div className="w-full cursor-pointer">
                        ...
                        <div className='w-full cursor-pointer'>
                          <div className="h-24 rounded-xl border border-dashed border-[var(--border-light)] bg-white grid place-items-center hover:border-[var(--cookd-orange)] hover:bg-[var(--cookd-orange)]/5 transition-all" onClick={() => handleCardClick(position)}>
                            <div className="text-xl text-[var(--text-muted)]">$75</div>
                          </div>
                          <div className="mt-2 text-xs text-center text-[var(--text-muted)]">{position}</div>
                        </div>
                    )}
...
function ProjectCard({ project, isPremium = false, isSmall = false }: { 
  ...
  if (isSmall) {
    return (
      <div className="card-hover card-base p-2 rounded-xl h-24 flex flex-col overflow-hidden">
        {project.image_url ? (
          <div className="relative w-full h-12 rounded-lg overflow-hidden flex-shrink-0">
            <Image
          ...
          <div className="w-full h-12 bg-gradient-to-br from-[var(--cookd-orange)]/10 to-[var(--cookd-golden)]/10 rounded-lg flex-shrink-0"></div>
        )}
        <h3 className="text-xs font-bold mt-1 text-[var(--text-primary)] truncate">{project.name}</h3>
      </div>
    );
  }
  ...
  return (
    <div className={`card-hover card-base p-4 rounded-xl overflow-hidden ${
      isPremium 
        ? 'border-4 border-[var(--cookd-golden)]' 
        : isStandard 
          ? 'border-2 border-[var(--cookd-golden)]/50' 
          : ''
    } ${isStandard ? 'h-[280px]' : ''} flex flex-col`}>
      {project.image_url ? (
        <div className={`relative w-full ${isPremium ? 'h-40' : 'h-28'} rounded-xl overflow-hidden flex-shrink-0`}>
          <Image
            src={project.image_url}
            alt={project.name}
            fill
        ...
        <div className={`w-full ${isPremium ? 'h-40' : 'h-28'} bg-gradient-to-br from-[var(--cookd-orange)]/10 to-[var(--cookd-golden)]/10 rounded-xl flex-shrink-0`}></div>
      )}
      
      {/* Creator Profile */}
      {showProfile && (
        <div className="flex items-center gap-2 mt-3">
          <div className="w-5 h-5 rounded-full bg-[var(--surface-cream)] overflow-hidden flex items-center justify-center flex-shrink-0 border border-[var(--border-light)]">
      ...
      <h3 className={`${isPremium ? 'text-xl' : 'text-base'} font-bold mt-2 text-[var(--text-primary)] line-clamp-1 font-display`}>{project.name}</h3>
      
      {project.description && (
        <p className="text-[var(--text-secondary)] mt-1 text-xs line-clamp-2 flex-1">{project.description}</p>
      )}
      ...
      <div className="flex gap-2 mt-3">
        ...
        {project.github_url && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="btn-inverse flex-1 text-center text-xs py-2"
            onClick={(e) => e.stopPropagation()}
          >
            GitHub
          </a>
